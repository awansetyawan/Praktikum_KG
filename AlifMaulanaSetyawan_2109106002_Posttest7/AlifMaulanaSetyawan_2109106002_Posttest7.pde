...
SoundFile musik; // Variabel musik
...
boolean isScaling = false; // Nilai untuk menentukan scaling mars dan bumi
...
boolean rotatePortal = false; // Kondisi awal menentukan rotate portal
...
void setup() {
  ...
  musik = new SoundFile(this, "EliminationRound.mp3");
...
void drawPortal() {
  ...
  // Kondisi untuk mengubah nilai rotatePortal, yang dimana enter untuk menyalakan dan backspace untuk mematikan
  if (keyPressed){
    if (key == ENTER){
      rotatePortal = true;
...
void drawBumi(){
  ...
  // Membuat kondisi dimana ketika nilai isScaling (menentukan scaling atmosfir bergerak atau tidak) berubah karena di trigger oleh mouseClicked
  if (isScaling) {
